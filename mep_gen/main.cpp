...
{    
    ...
    while(true){
        mep_seqno = random_increment(mep_seqno);
        payload = Tools::toString<int>(mep_seqno);
    ...
    if (pkt_type == "udp"){
        ...
        if (libnet_build_udp(
                10000, 10000, 8 /* size of the udp headers */ + payload.length(), 0, (u_int8_t*)payload.c_str(), payload.length(), l, 0) == -1)
        {
...
int random_increment(int them_numbers){
    srand ( time(NULL) + them_numbers );
    
    // TODO: Do the test with numbers out of order as well
    int r = (rand() % 2) + 1;
    
    return them_numbers + r;
}